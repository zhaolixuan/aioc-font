...
function findIntervals(waringData, zoneList) {
    // 将字符串a拆分为两个数字
    const { startPosition, endPosition,channelName } = waringData
    // 初始化结果数组
    const result = [];
    ...
    for (let i = 0; i < zoneList.length; i++) {
        ...
        const channelName1= zoneList[i].channelName
        // 如果start和end都在当前区间内，则将当前区间添加到结果数组中
        if (((startPosition >= channelStartNum && startPosition< channelEndNum ) ||  (endPosition > channelStartNum &&  endPosition <= channelEndNum) )&& channelName == channelName1) {
            result.push({

...
import roads from './data/nanchang/roads.json'
...
export default {
  ...
  data() {
    return {
      ...
      center: [115.95711915969923, 28.667006341810264],
  ...
  methods: {
    initMapTalksMap() {
      ...
      this.mapboxBaseLayer = new MapboxglLayer('tile', {
        glOptions: {
          style: 'mapbox://styles/tangjuncheng/cl3v2bhrm000a16s3pgw9ta5k',
        },
      })
    ...
    addCenterPoints() {
      ...
      api.getfindListByTimeList().then((res) => {
        var data = res.data
          ...
          if (item !== undefined) {
            ...
            var point = new maptalks.Marker(lnglat, {
              properties: {
                ...
                value: value,
                wllse: item.sumSalesamount,
                wllsj: item.sumSellcount,
                dpsl: item.shopNum,
                type: 'other',
              },
              symbol: [
    ...
    async addThreeLayer() {
      ...
      function addRippleWall() {
        var boundMax = require(`./data/nanchang/bound.json`)

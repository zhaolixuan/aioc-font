...
const R = 6378137
const MINLENGTH = 1
var formatLineArray = function(polyline) {
...
var lineLength = function(polyline) {
...
var getLngLatKey = function(lnglat) {
...
var getPercentLngLat = function(l, length) {
  const { len, c1, c2 } = l
  const dx = c2[0] - c1[0],
    dy = c2[1] - c1[1],
    dh = (c2[2] || 0) - (c1[2] || 0)
...
var lineSlice = function(cs, lineChunkLength = 10) {
...
var getLinePosition = function(lineString, layer, cenerter) {
  const positions = []
  const positionsV = []
...
var getChunkLinesPosition = function(chunkLines, layer, positionMap, cenerPt) {
  const positions = [],
    positionsV = [],
    lnglats = []
...
var setLineGeometryAttribute = function(geometry, ps) {
  const len = ps.length
...
var _getLinePosition = function(lineString, layer) {
  const positions = []
  const positionsV = []
  if (Array.isArray(lineString) && lineString[0] instanceof THREE.Vector3) {
    for (let i = 0, len = lineString.length; i < len; i++) {
...
var _getChunkLinesPosition = function(chunkLines, layer) {
  const positions = [],
    positionsV = [],
    lnglats = []
  for (let i = 0, len = chunkLines.length; i < len; i++) {
...
export {
  ...
  lineSlice,
  getLinePosition,
  getChunkLinesPosition,
  setLineGeometryAttribute,
  _getLinePosition,
  _getChunkLinesPosition,
}
